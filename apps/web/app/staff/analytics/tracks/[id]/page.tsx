...
import { ApiError, fetchTrackAnalytics, type TrackAnalyticsRecord } from "@/lib/api";
import {
...
function TrackAnalyticsBody() {
  ...
  const [data, setData] = useState<TrackAnalyticsRecord | null>(null);
  ...
  useEffect(() => {
    ...
    setErrorKind(null);
    setData(null);
    void fetchTrackAnalytics(params.id, fromParam || null, toParam || null)
      .then((body) => {
        if (!cancelled) {
          setData(body);
        }
      ...
      .catch((err: unknown) => {
        ...
        if (err instanceof ApiError) {
          ...
          setErrorKind("other");
          setError("Unable to load track analytics");
        }
      })
      .finally(() => {
        if (!cancelled) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold">Track analytics</h1>
        <p className="text-sm text-neutral-600">
          All plays from analytics facts. Royalty-eligible plays are not computed here.
        </p>
        <p className="text-sm">
          <Link href={`/tracks/${params.id}`} className="underline">
            Open track
          </Link>
        </p>
      </div>
        ...
        <>
          <PeriodEmptyNote
            hasCounts={periodHasCounts([
              ...
              data.listen_duration_ms,
            ])}
          />
          <section className="grid gap-3 sm:grid-cols-2">
            <MetricTile label="Plays" value={data.play_count} loading={false} />
            <MetricTile label="Completed plays" value={data.completed_play_count} loading={false} />
            <MetricTile label="Listen duration (ms)" value={data.listen_duration_ms} loading={false} />
            <MetricTile
